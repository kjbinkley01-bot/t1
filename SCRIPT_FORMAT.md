...
* `settings.glide_ms`: slide the mouse to each position over this many milliseconds (eased, up to 5000)
  instead of jumping there. 0 jumps. It applies to clicks, Click Image, drags, scrolls at a position and
  every Move Mouse step, and runs faster or slower with `speed`. `glide_curve` bends each slide slightly
  to one side, like a hand moving the mouse.
